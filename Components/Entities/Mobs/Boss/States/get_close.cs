...
public class BossGetCloseState : State
{
    public new Boss parent_node;
    public player player_node;
    public double waiting_time = 0;
    public BossGetCloseState(Boss parent_node) : base(parent_node)
    {
    ...
    override public State _process_state(double delta) {
        ...
        if(parent_node.GlobalPosition.DistanceTo(player_node.GlobalPosition) > 180) {
            parent_node.movement_speed_vector = GetMovementToPlayerSpeed();
        } else if (parent_node.GlobalPosition.DistanceTo(player_node.GlobalPosition) < 150) {
