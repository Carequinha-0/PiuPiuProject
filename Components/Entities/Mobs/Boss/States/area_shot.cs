...
public class BossAreaShotState : State
{
    ...
    public BossAreaShotState(Boss parent_node) : base(parent_node)
    {
    ...
    override public State _process_state(double delta) {
        ...
        if(shot_current_cooldown > SHOT_COOLDOWN) {
            ...
            var direction = parent_node.GlobalPosition.DirectionTo(player_node.GlobalPosition);
